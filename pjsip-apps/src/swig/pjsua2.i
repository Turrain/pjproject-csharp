...
// Extend ByteVector with optimized copy methods
%extend std::vector<unsigned char> {
  void CopyToArray(unsigned char *dest, unsigned int len) {
    if (len > $self->size()) {
      len = (unsigned int)$self->size();
    }
    memcpy(dest, &(*$self)[0], len);
  }
  
  void CopyFromArray(unsigned char *src, unsigned int len) {
    $self->resize(len);
    memcpy(&(*$self)[0], src, len);
  }
}
#ifdef SWIGCSHARP
%typemap(csimports) std::vector<unsigned char> %{
using System;
using System.Runtime.InteropServices;
%}

%typemap(cscode) std::vector<unsigned char> %{
  public void CopyFrom(byte[] source) {
    if (source == null)
      throw new ArgumentNullException(nameof(source));

    // Pin the array so it doesn't move in memory during the operation
    GCHandle handle = GCHandle.Alloc(source, GCHandleType.Pinned);
    try {
      IntPtr ptr = handle.AddrOfPinnedObject();
      SWIGTYPE_p_unsigned_char srcPtr = new SWIGTYPE_p_unsigned_char(ptr, false);
      CopyFromArray(srcPtr, (uint)source.Length);
    }
    finally {
      handle.Free();
    }
  ...
  public static ByteVector FromBytes(byte[] source) {
    if (source == null)
      throw new ArgumentNullException(nameof(source));
  ...
  public void CopyToBytes(byte[] destination) {
    if (destination == null) {
      throw new ArgumentNullException(nameof(destination));
    }
    
    if (this.Count <= 0 || destination.Length <= 0) {
    ...
    try {
      IntPtr ptr = handle.AddrOfPinnedObject();
      SWIGTYPE_p_unsigned_char destPtr = new SWIGTYPE_p_unsigned_char(ptr, false);
      CopyToArray(destPtr, (uint)destination.Length);
    }
    finally {
      handle.Free();
    }
  ...
  public ByteVector Clone() {
    // Use the existing C++ vector copy constructor for efficient copying
  ...
  public byte[] ToByteArray() {
    ...
    try {
      ...
      // If copying fails, return empty array rather than throwing
      Console.WriteLine("Warning: Failed to copy ByteVector to byte array");
    }
